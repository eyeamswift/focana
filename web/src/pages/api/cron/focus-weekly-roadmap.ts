...
async function handle(request: Request) {
  ...
  const weekStart = url.searchParams.get('weekStart') || getPreviousIsoWeekStart();
  ...
  try {
    ...
    const result = await sendWeeklyFocusRoadmaps({ store, loopsApiKey, weekStart });
    return json(result);
  } catch (error) {
    console.error('[focus-weekly-roadmap] Unexpected error:', error);
    return json({ error: 'Failed to process weekly focus roadmap' }, 500);
  }
}
