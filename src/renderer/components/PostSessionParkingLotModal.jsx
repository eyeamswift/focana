...
export default function PostSessionParkingLotModal({
  ...
  const handleConfirmDelete = () => {
    ...
    if (pendingDelete.type === 'single') {
      onDismissThought?.(pendingDelete.id);
            ...
            {thoughts.map((thought) => (
              ...
              >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <Button
                    onClick={() => setPendingDelete({ type: 'single', id: thought.id, label: thought.text })}
                    variant="outline"
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
                    onClick={() => setPendingDelete({ type: 'all', label: `${thoughts.length} visible item${thoughts.length === 1 ? '' : 's'}` })}
                    style={{ borderColor: '#FCA5A5', color: '#DC2626' }}
                  >
                    <Trash2 style={{ width: 14, height: 14, marginRight: '0.35rem' }} />
                    Clear all
        ...
        <DialogContent style={{ background: 'var(--bg-surface)', borderColor: 'var(--brand-action)', maxWidth: '26rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', lineHeight: 1.5 }}>
            {pendingDelete?.type === 'single'
              ? `This will permanently dismiss "${pendingDelete?.label || 'this note'}".`
              : `This will permanently dismiss ${pendingDelete?.label || 'these items'}.`}
          </p>
          <DialogFooter style={{ marginTop: '1rem', justifyContent: 'flex-end', gap: '0.5rem' }}>
            ...
            <Button onClick={handleConfirmDelete} style={{ background: 'var(--error)', color: 'white' }}>
              {pendingDelete?.type === 'single' ? 'Dismiss Note' : 'Clear All'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
