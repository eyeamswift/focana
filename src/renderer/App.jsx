...
const APP_ACTIVE_DAY_STORAGE_KEY = 'focana-app-active-day';
const WINDOW_SIZES = {
  baseWidth: 460,
  runningWidth: 432,
  idleHeight: 124,
...
function getReentryPromptHeightForStage(stage) {
  if (stage === 'resume-choice') return WINDOW_SIZES.reentryPromptStageHeights.resumeChoice;
  if (stage === 'start-chooser') return WINDOW_SIZES.reentryPromptStageHeights.startChooser;
  if (stage === 'save-for-later') return WINDOW_SIZES.reentryPromptStageHeights.saveForLater;
  if (stage === 'snooze-options') return WINDOW_SIZES.reentryPromptStageHeights.snoozeOptions;
...
function formatNumericMonthDay(value) {
...
export default function App() {
  ...
  const [reentryBreakReturnAvailable, setReentryBreakReturnAvailable] = useState(false);
  const [reentryReturnAfterModal, setReentryReturnAfterModal] = useState(false);
  ...
  const pomodoroBreakReturnToCompactRef = useRef(false);
  const startChooserReturnToCompactRef = useRef(null);
  ...
  const postSessionNotesActionRef = useRef(null); // 'resume-later' | 'complete-direct' | null
  const directCompletionNotesSourceRef = useRef(null);
  ...
  const showSurfaceReentryPromptRef = useRef(false);
  ...
  const reentryPromptVisibleBeforeModalRef = useRef(false);
  const reentryForceRestoreAfterModalRef = useRef(false);
  const reentryModalWasOpenRef = useRef(false);
  ...
  const updateReentryBreakReturnAvailable = useCallback((available) => {
    const nextAvailable = available === true;
    reentryBreakReturnAvailableRef.current = nextAvailable;
    setReentryBreakReturnAvailable(nextAvailable);
  }, []);

  const scheduleReentryCueFromNow = useCallback((delayMs = REENTRY_DELAY_MS) => {
    const now = Date.now();
    const safeDelayMs = Math.max(0, Math.floor(Number(delayMs) || 0));
    reentryEligibleSinceRef.current = now;
    reentryNextCueAtRef.current = now + safeDelayMs;
  ...
  const handleTaskChange = useCallback((nextValue) => {
    ...
    setTaskPlan((prev) => {
      const nextPlan = syncActiveTaskTitle(prev, normalizedValue);
  ...
  const trackDailyAppActive = useCallback((source = 'unknown') => {
    if (typeof window === 'undefined') return false;

    const localDay = getLocalDateKey();
    try {
      const previousDay = window.localStorage.getItem(APP_ACTIVE_DAY_STORAGE_KEY);
      if (previousDay === localDay) return false;
      window.localStorage.setItem(APP_ACTIVE_DAY_STORAGE_KEY, localDay);
    } catch (_) {
      // If localStorage is unavailable, still emit the active signal.
    }
    ...
    track('app_active', {
      ...
      timer_visible: Boolean(isTimerVisible),
    });
    return true;
  }, [isRunning, isTimerVisible, mode]);
  ...
  useEffect(() => {
    ...
    track('app_opened');
    trackDailyAppActive('startup_ready');
    ...
    const updateInteraction = () => {
      ...
      if (now - lastInteractionPingAt < 60 * 1000) return;
      lastInteractionPingAt = now;
      trackDailyAppActive('interaction');
    };
    const handleFocus = () => {
      trackDailyAppActive('window_focus');
    };
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        trackDailyAppActive('window_visible');
      }
    };
    const activeSessionInterval = window.setInterval(() => {
      if (isRunningRef.current) {
        trackDailyAppActive('running_session');
      }
    }, 60 * 1000);
    ...
    return () => {
      window.clearInterval(activeSessionInterval);
  ...
  }, [startupGateState, startupRevealComplete, trackDailyAppActive]);
  ...
  const handlePostSessionLayoutChange = useCallback(() => {
    setPostSessionLayoutVersion((prev) => prev + 1);
  }, []);
  ...
  const reentryParkingLotItems = useMemo(() => thoughts
    ...
    .map((thought, index) => ({
      ...
      meta: formatNumericMonthDay(thought.createdAt),
      thought,
  ...
  useEffect(() => {
    ...
    reentryPromptVisibleBeforeModalRef.current = true;
    if (Number.isFinite(reentryNextCueAtRef.current)) return;
    scheduleReentryCueFromNow();
  }, [scheduleReentryCueFromNow, showSurfaceReentryPrompt]);

  useEffect(() => {
    if (!showSurfaceReentryPrompt) return;
    if (!showHistoryModal && !distractionJarOpen) return;
    reentryReturnAfterModalRef.current = true;
    setReentryReturnAfterModal(true);
  }, [distractionJarOpen, showHistoryModal, showSurfaceReentryPrompt]);
  ...
  const resumeActiveTimer = useCallback((source = 'unknown') => {
    ...
    track('session_resumed', {
      source,
      mode,
      elapsed_minutes: resumedElapsedMinutes,
    });
    if (!currentSessionId) {
  ...
  }, [currentSessionId, initialTime, mode, trackDailyAppActive, updateReentryBreakReturnAvailable]);
  ...
  const ensureCurrentSessionId = useCallback(async (source = 'unknown') => {
    ...
    return sessionCreatePromiseRef.current;
  }, [currentSessionId, task, mode, contextNotes, loadSessions, nextStepsNotes, persistableTaskPlan]);
  ...
  const saveSessionWithNotes = useCallback(async (notes) => {
    ...
    const sessionTaskPlan = prepareTaskPlanForStart(sessionToSave.current?.taskPlan || taskPlan, task);
    ...
    try {
      if (duration > 0.1) {
        ...
        if (kept || completed) {
          // Explicit keep/complete choices should preserve the shell session
          ...
          // Preserve existing behavior for discarded very short sessions.
          await SessionStore.delete(activeSessionId);
          savedSessionId = null;
        }
      } else if (kept || completed) {
        // If the async shell session has not landed yet, still persist the
  ...
  }, [currentSessionId, task, mode, contextNotes, getElapsedSeconds, ensureCurrentSessionId, nextStepsNotes, patchSessionInLocalState, persistableTaskPlan]);
  ...
  const captureSessionFeedback = useCallback(async (feedback, promptOverride = null) => {
  ...
  }, [ensureCurrentSessionId, licenseStatus?.installId, licenseStatus?.instanceId, runtimeInfo?.channel, runtimeInfo?.osVersion, runtimeInfo?.version]);
  ...
  const clearPostSessionSurfaceForSystemEntry = useCallback(() => {
    ...
    postSessionReturnToFloatingRef.current = false;
  ...
  const presentSystemEntryPrompt = useCallback((source = systemEntryPendingRef.current?.source) => {
    ...
    clearPostSessionSurfaceForSystemEntry();
  ...
  const reconcilePendingSystemEntryReveal = useCallback(async () => {
    if (startupSetupGateActive) return false;
    try {
      const source = await window.electronAPI.getPendingSystemEntryReveal?.();
      if (source !== 'login' && source !== 'wake-resume' && source !== 'wake-login') return false;
      return presentSystemEntryPrompt(source);
    } catch (error) {
      console.warn('Failed to reconcile pending system-entry reveal:', error);
      return false;
    }
  }, [presentSystemEntryPrompt, startupSetupGateActive]);
  ...
  const resetReentryAttention = useCallback(({ preserveSnooze = false, preserveSystemEntry = false } = {}) => {
    ...
    setReentryAttentionVisible(false);
    reentrySurfaceSignatureRef.current = '';
    reentryPausedVisibleByBlockerRef.current = false;
    reentryPromptVisibleBeforeModalRef.current = false;
    reentryReturnAfterModalRef.current = false;
    setReentryReturnAfterModal(false);
    updateReentryBreakReturnAvailable(false);
    closeFloatingReentryPrompt();
  ...
  }, [clearSystemEntryPending, closeFloatingReentryPrompt, updateReentryBreakReturnAvailable]);
  ...
  const settleReentryCueAfterInteraction = useCallback(() => {
    const now = Date.now();
    if (reentryStrongTimeoutRef.current) {
      clearTimeout(reentryStrongTimeoutRef.current);
      reentryStrongTimeoutRef.current = null;
    ...
    setReentryStrongActive(false);
    reentryNextCueAtRef.current = now + REENTRY_DELAY_MS;
    reentryRemainingMsRef.current = null;
  }, []);
  ...
  const pauseReentryAttention = useCallback((now = Date.now()) => {
    const hadVisibleReentry = reentryAttentionVisibleRef.current
      || showSurfaceReentryPromptRef.current
      || reentryFloatingPromptOpenRef.current;
    if (hadVisibleReentry) {
      reentryPausedVisibleByBlockerRef.current = true;
    }
    if (reentryStrongTimeoutRef.current) {
      clearTimeout(reentryStrongTimeoutRef.current);
      reentryStrongTimeoutRef.current = null;
    ...
    } else if (reentryAttentionVisibleRef.current) {
  ...
  const restoreReentryPromptAfterModal = useCallback((force = false) => {
    if (
      !force
      && !reentryReturnAfterModal
      && !reentryReturnAfterModalRef.current
      && !reentryPausedVisibleByBlockerRef.current
      && !reentryPromptVisibleBeforeModalRef.current
    ) return false;
    reentryReturnAfterModalRef.current = false;
    reentryPausedVisibleByBlockerRef.current = false;
    reentryPromptVisibleBeforeModalRef.current = false;
    reentryForceRestoreAfterModalRef.current = false;
    setReentryReturnAfterModal(false);
    const now = Date.now();
    if (reentryStrongTimeoutRef.current) {
      clearTimeout(reentryStrongTimeoutRef.current);
    ...
    closeFloatingReentryPrompt();
    return true;
  }, [closeFloatingReentryPrompt, reentryReturnAfterModal]);

  useEffect(() => {
    const reentryModalOpen = showHistoryModal || distractionJarOpen;
    if (reentryModalWasOpenRef.current && !reentryModalOpen) {
      restoreReentryPromptAfterModal();
    }
    reentryModalWasOpenRef.current = reentryModalOpen;
  }, [distractionJarOpen, restoreReentryPromptAfterModal, showHistoryModal]);
  ...
  const handleExitCompact = useCallback(() => {
    ...
    pendingCompactExitHeightRef.current = showSurfaceReentryPrompt
      ? getReentryPromptHeightForStage(reentrySurfaceStage)
      : exitTargetHeight;
  }, [checkInState, hasSavedContext, isCompact, isRunning, isStartModalOpen, isTimerVisible, reentrySurfaceStage, showSurfaceReentryPrompt, task]);
  ...
  const openHistoryFromReentry = useCallback(({ bringToFront = false } = {}) => {
    reentryReturnAfterModalRef.current = true;
    reentryForceRestoreAfterModalRef.current = true;
    setReentryReturnAfterModal(true);
    if (bringToFront) {
  ...
  const openParkingLotFromReentry = useCallback(({ bringToFront = false } = {}) => {
    reentryReturnAfterModalRef.current = true;
    reentryForceRestoreAfterModalRef.current = true;
    setReentryReturnAfterModal(true);
    if (bringToFront) {
  ...
  }, [task, isRunning, isCompact, handleExitCompact, pauseActiveTimer, resumeActiveTimer]);
  ...
  }, [handleExitCompact, isCompact, requestCompactEntry]);

  const handleShortcutCompleteTask = useCallback(async () => {
    await completeActiveSessionDirectRef.current({ source: 'shortcut', bringToFront: true });
  }, []);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        pomodoroBreakReturnToCompactRef.current = Boolean(savedTimerState?.pomodoroBreakReturnToCompact);
  ...
  useEffect(() => {
    ...
    if (!isRunning) {
      ...
      window.electronAPI.storeSet('timerState', {
        ...
        pomodoroBreakReturnToCompact: pomodoroBreakReturnToCompactRef.current,
  ...
  const persistIdleTimerSnapshot = useCallback(({
    ...
    window.electronAPI.storeSet('timerState', {
      ...
      pomodoroBreakReturnToCompact: false,
  ...
  const applyPausedTimerSnapshot = useCallback((payload = {}) => {
    ...
    pomodoroBreakReturnToCompactRef.current = Boolean(nextTimerState?.pomodoroBreakReturnToCompact);
  ...
  const resetFocusRhythmState = useCallback(() => {
    ...
    pomodoroBreakIntentionRef.current = '';
    pomodoroBreakReadyToResumeRef.current = false;
    pomodoroBreakReturnToCompactRef.current = false;
  ...
  const handleCheckInFinished = useCallback(async () => {
    ...
    clearCheckInUi();
    await completeActiveSessionDirectRef.current({
      source: 'checkin',
      elapsedSeconds: elapsedSec,
      bringToFront: true,
    });
  }, [clearCheckInUi, getElapsedSeconds, logCheckIn]);
  ...
  const reentryModalBlockerOpen = (reentryReturnAfterModal || reentryReturnAfterModalRef.current)
    && (showHistoryModal || showTaskPreview || distractionJarOpen);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (cancelled) return;
      void reconcilePendingSystemEntryReveal();
    };
  ...
  }, [clearPostSessionSurfaceForSystemEntry, clearSystemEntryPending, presentSystemEntryPrompt, startupSetupGateActive]);
    ...
    const tick = async () => {
      ...
      if (showSurfaceReentryPromptRef.current || reentryFloatingPromptOpenRef.current) {
        if (!Number.isFinite(reentryNextCueAtRef.current)) {
          reentryEligibleSinceRef.current = now;
          reentryNextCueAtRef.current = now + REENTRY_DELAY_MS;
        } else if (!reentryStrongActiveRef.current && now >= reentryNextCueAtRef.current) {
          fireReentryCue();
          reentryNextCueAtRef.current = now + REENTRY_LOOP_MS;
        }
        reentryRemainingMsRef.current = null;
      ...
      if (postSessionBreakPromptPendingRef.current) {
        postSessionBreakPromptPendingRef.current = false;
        setFloatingBreakState({ open: false });
        updateReentryBreakReturnAvailable(true);
        reentryEligibleSinceRef.current = now;
        reentryNextCueAtRef.current = now + REENTRY_LOOP_MS;
        reentryRemainingMsRef.current = null;
        fireReentryCue();
  ...
  }, [time, isRunning, mode, initialTime, getElapsedSeconds, getTimedCueSegmentDuration, getTimedCueSegmentElapsed, isCompact, pulseSettings.compactEnabled, dndEnabled, hasBlockingWindowOpen, triggerPulse]);
  ...
  const openPomodoroWorkTimeChooser = useCallback(() => {
    const carryoverSeconds = Math.max(0, Math.floor(Number(elapsedBeforeRunRef.current) || 0));
    ...
    setPomodoroBreakIntention('');
    setPomodoroBreakReadyToResume(false);
    setMode(TIMER_MODES.FREEFLOW);

    const chooserOpened = prepareTaskForStartChooserRef.current({
      taskText: task,
      recap: contextNotes,
      nextSteps: nextStepsNotes,
      project,
      ...
      sessionId: currentSessionId,
      carryoverSeconds,
      returnToCompact: pomodoroBreakReturnToCompactRef.current,
    });
    if (!chooserOpened) return;

    track('pomodoro_work_time_chooser_shown', {
      completed_cycles: pomodoroCyclesCompletedRef.current,
    });
    window.setTimeout(() => {
      sessionMinutesInputRef.current?.focus();
  ...
  }, [contextNotes, currentSessionId, nextStepsNotes, persistableTaskPlan, project, task]);
  ...
  const presentPomodoroBreakSurface = useCallback((source = 'work-complete') => {
    ...
    if (source === 'work-complete') {
      pomodoroBreakReturnToCompactRef.current = compactModeActive;
    }
  ...
  useEffect(() => {
    ...
    const showTimeUpFlow = async () => {
      ...
      const returnToFloating = Boolean(restoredFromFloating);
      const returnToCompact = returnToFloating ? false : wasCompact;
      const durationMin = initialTime / 60;
      ...
      const endedSessionId = await saveSessionWithNotes({
        recap: contextNotes,
        nextSteps: nextStepsNotes,
      });
      openPostSessionTransitionRef.current({
        taskText: task,
        recap: contextNotes,
        nextSteps: nextStepsNotes,
        sessionId: endedSessionId,
        carryoverSeconds: 0,
        completedMinutes: durationMin,
        completedMode: 'timed',
        returnToCompact,
        returnToFloating,
      });
  ...
  const handleCloseParkingLot = useCallback(() => {
    ...
    const shouldForceRestoreIdleReentry = reentryForceRestoreAfterModalRef.current
      && !task.trim()
      && !isRunning
      && !isTimerVisible
      && reentrySurfaceStage === 'task-entry';
    parkingLotReturnToCompactRef.current = false;
    parkingLotReturnToFloatingRef.current = false;
    if (!reopenedPrevModal && restoreReentryPromptAfterModal(shouldForceRestoreIdleReentry)) {
      return;
    }
  ...
  }, [isRunning, isTimerVisible, popAndOpenPrevModal, reentrySurfaceStage, restoreDisplayMode, restoreReentryPromptAfterModal, task]);
  ...
  const measureMainCardContentHeight = useCallback(() => {
    const card = mainCardRef.current;
    if (!card) return 0;

    const cardRect = card.getBoundingClientRect();
    const computedStyles = window.getComputedStyle(card);
    const paddingBottom = Number.parseFloat(computedStyles.paddingBottom || '0') || 0;
    const childElements = Array.from(card.children).filter((child) => child instanceof HTMLElement);

    if (childElements.length === 0) {
      return Math.ceil(cardRect.height || 0);
    }

    const contentBottom = childElements.reduce((maxBottom, child) => {
      const childRect = child.getBoundingClientRect();
      return Math.max(maxBottom, childRect.bottom);
    }, cardRect.top);

    return Math.ceil(Math.max(0, (contentBottom - cardRect.top) + paddingBottom));
  }, []);

  const resizeToMainCardContent = useCallback((minHeight) => {
    const measuredHeight = measureMainCardContentHeight();
    const targetHeight = Math.max(minHeight, measuredHeight);
    ensureWindowSizeForCurrentScreen(targetHeight);
  }, [ensureWindowSizeForCurrentScreen, measureMainCardContentHeight]);
  ...
  }, [hasActiveResumeCandidate, scheduleReentryCueFromNow, sessionStateHydrated, startupGateState, startupLaunchSource, startupLaunchSourceHydrated, startupRevealComplete]);
  ...
  useLayoutEffect(() => {
    ...
    const syncSettled = window.setTimeout(() => {
      resyncFullWindowSize();
    }, 120);
  ...
  useEffect(() => {
    ...
    if (showSurfaceReentryPrompt) {
      const targetHeight = getReentryPromptDefaultHeight();
      ensureWindowSizeForCurrentScreen(targetHeight);
    ...
    return () => clearTimeout(t);
  }, [ensureWindowSizeForCurrentScreen, getReentryPromptDefaultHeight, hasSavedContext, showSurfaceReentryPrompt, startupRevealComplete, startupGateState, isCompact, isRunning, isTimerVisible, isStartModalOpen, task]);
  ...
  const handleStop = useCallback(async (options = {}) => {
    ...
    const splitNotes = normalizeSplitNotes({
      recap: contextNotes,
      nextSteps: nextStepsNotes,
    });
    clearCompactSessionCues();
    ...
    if (pendingParkingLotTaskSwitchRef.current?.taskText) {
      sessionToSave.current = {
      ...
      await saveSessionWithNotes(splitNotes);
      elapsedBeforeRunRef.current = 0;
      setIsRunning(false);
      setTime(0);
      setInitialTime(0);
      ...
      setIsCompact(false);
      setSessionStartTime(null);
      startPendingParkingLotTaskSwitch();
      return;
    }
  ...
  }, [clearCompactSessionCues, contextNotes, currentSessionId, getElapsedSeconds, handleExitCompact, isCompact, isRunning, mode, nextStepsNotes, pauseActiveTimer, saveSessionWithNotes, startPendingParkingLotTaskSwitch, task]);
  ...
  }, [reconcilePausedTimerSnapshotFromStore, reconcilePendingSystemEntryReveal]);
  ...
  const handleTaskSubmit = (submittedTask = task) => {
    ...
    if (nextTask !== task) {
      setTask(nextTask);
      setTaskPlan((prev) => {
        const nextPlan = syncActiveTaskTitle(prev, nextTask);
        taskPlanRef.current = nextPlan;
        return nextPlan;
      });
    }
  ...
  const handleStartSession = async (selectedMode, minutes, options = {}) => {
    ...
    const chooserReturnToCompact = startChooserReturnToCompactRef.current;
    startChooserReturnToCompactRef.current = null;
    const shouldReturnToFloating = options?.returnToFloating === true;
    const shouldReturnToCompact = shouldReturnToFloating
      ? false
      : typeof options?.returnToCompact === 'boolean'
        ? options.returnToCompact
        : typeof chooserReturnToCompact === 'boolean'
          ? chooserReturnToCompact
          : true;
  ...
  const finishDirectCompletion = useCallback(async ({
    ...
    if (pendingTaskPlanTransition?.nextTask?.title) {
      const nextPlan = setActiveTask(
        ...
        pendingTaskPlanTransition.nextTask.id,
      );
      const nextTaskTitle = pendingTaskPlanTransition.nextTask.title;
      setTask(nextTaskTitle);
      setTaskPlan(nextPlan);
      setTaskPlanCompletionDismissedFor(null);
      setTaskPlanTransition({
        completedTask: pendingTaskPlanTransition.completedTaskTitle,
      ...
      persistIdleTimerSnapshot({
        taskText: nextTaskTitle,
        recapText: '',
        nextStepsText: '',
        taskPlanSnapshot: nextPlan,
        nextMode: 'freeflow',
        sessionId: null,
      });
      if (bringToFront) {
  ...
  const completeActiveSessionDirect = useCallback(async ({
    ...
    const completedTaskPlan = activePlanTaskForCompletion?.id
      ...
      : preparedTaskPlan;
    const nextPlanTask = getNextUnfinishedTask(completedTaskPlan);
    taskPlanTransitionAfterCompletionRef.current = nextPlanTask
      ? {
        taskPlan: completedTaskPlan,
        completedTaskTitle: activePlanTaskForCompletion?.title || task,
        nextTask: nextPlanTask,
      }
      : null;
    setTaskPlan(completedTaskPlan);
    setTaskPlanCompletionDismissedFor(null);

    const hasExplicitElapsedSeconds = Number.isFinite(Number(elapsedSeconds));
    const normalizedElapsedSeconds = hasExplicitElapsedSeconds
      ? Math.max(0, Number(elapsedSeconds))
      : (isRunning ? pauseActiveTimer() : getElapsedSeconds());
    if (hasExplicitElapsedSeconds && isRunning) {
      pauseActiveTimer();
    }
    const durationMin = normalizedElapsedSeconds / 60;
    const splitNotes = normalizeSplitNotes(notes, {
      recap: contextNotes,
      nextSteps: nextStepsNotes,
    });
    const completionSessionId = currentSessionId || await ensureCurrentSessionId('direct completion');
    ...
    const settings = await window.electronAPI.storeGet('settings') || {};
    if (settings.keepTextAfterCompletion ?? false) {
      directCompletionNotesSourceRef.current = {
        source,
        durationMin,
        completedMode: mode,
        bringToFront,
      };
      postSessionNotesActionRef.current = 'complete-direct';
      if (isCompact) {
        handleExitCompact();
      }
      setShowPostSessionPrompt(false);
  ...
  const handleTaskPlanChange = useCallback((nextPlan) => {
    ...
    taskPlanRef.current = syncedPlan;
    setTaskPlan(syncedPlan);
    setTaskPlanCompletionDismissedFor(null);
  }, [task]);

  const handleRunningTaskPlanChange = useCallback((nextPlan) => {
    const syncedPlan = syncActiveTaskTitle(nextPlan, task);
    taskPlanRef.current = syncedPlan;
    setTaskPlan(syncedPlan);
    setTaskPlanCompletionDismissedFor(null);
    ...
    if (currentSessionId) {
      void SessionStore.update(currentSessionId, {
        taskPlan: prepareTaskPlanForStart(syncedPlan, task),
      }).then(() => loadSessions()).catch((error) => {
        console.error('Failed to persist running plan edit:', error);
      });
    }
  ...
  const handleKeepActiveTaskGoing = useCallback(() => {
    ...
    if (active?.id) setTaskPlanCompletionDismissedFor(active.id);
  }, [persistableTaskPlan]);

  const handleMarkActivePlanTaskComplete = useCallback(() => {
    void completeActiveSessionDirect({
      source: 'task_plan',
      taskPlanSnapshot: persistableTaskPlan,
    });
  }, [completeActiveSessionDirect, persistableTaskPlan]);

  const beginStartSomethingNewFromResumeCandidate = useCallback(() => {
    const candidate = reentryResumeCandidateRef.current;
    reentrySurfaceSignatureRef.current = `resume-choice:${candidate?.sessionId || candidate?.taskText || 'draft'}`;
    updateReentryBreakReturnAvailable(false);
    closeFloatingReentryPrompt();
    scheduleReentryCueFromNow();
    setReentryAttentionVisible(true);
    ...
    setReentrySurfaceTaskText(clampTaskText(candidate?.taskText || task));
    setIsCompact(false);
    window.electronAPI.bringToFront?.();
  }, [closeFloatingReentryPrompt, scheduleReentryCueFromNow, task, updateReentryBreakReturnAvailable]);
  ...
  const startSessionFromReentryPrompt = useCallback((payload = {}, { bringToFront = false } = {}) => {
    ...
    return true;
  }, [clearSystemEntryPending, closeFloatingReentryPrompt, handleStartSession, updateReentryBreakReturnAvailable]);
  ...
  const handleBackToBreakMode = useCallback(() => {
    const lastBreak = postSessionLastBreakOptionsRef.current || {};
    const breakMinutes = clampSessionMinutes(lastBreak.minutes, 5);
    const showTimer = lastBreak.showTimer === true;
    ...
    void window.electronAPI.enterFloatingMinimize?.();
  }, [closeFloatingReentryPrompt, setFloatingBreakState, updateReentryBreakReturnAvailable]);

  const handleFloatingBreakAction = useCallback((eventPayload = {}) => {
    const action = typeof eventPayload?.action === 'string' ? eventPayload.action : '';
    if (action !== 'resume-now') return;
  ...
  const handleFloatingReentryAction = useCallback((eventPayload = {}) => {
    ...
    if (action === 'back-to-break-mode') {
      handleBackToBreakMode();
      return;
    }
    ...
    if (action === 'open-history') {
      openHistoryFromReentry({ bringToFront: true });
      return;
    }

    if (action === 'open-parking') {
      openParkingLotFromReentry({ bringToFront: true });
      return;
    }
  ...
  const handleSurfaceReentryStart = useCallback((payload = {}) => {
    ...
    const nextPayload = {
      ...payload,
      taskPlan: payloadTaskPlan || reentrySurfaceTaskPlan,
    };
  ...
  const handleReentrySurfaceStageChange = useCallback((nextStage) => {
    settleReentryCueAfterInteraction();
    setReentrySurfaceStage(nextStage);
  }, [settleReentryCueAfterInteraction]);
  ...
  useEffect(() => {
    const cleanup = window.electronAPI.onFloatingBreakAction?.((payload) => {
      handleFloatingBreakAction(payload);
    });
    return () => { if (cleanup) cleanup(); };
  ...
  const openPostSessionTransition = useCallback(({
    ...
    returnToFloating = false,
    resolution = 'open',
    taskPlan: incomingTaskPlan = persistableTaskPlan,
  }) => {
    const candidate = buildPostSessionResumeCandidate({
      taskText,
      recap,
    ...
    setPostSessionStartAssist(false);
    setShowPostSessionPrompt(true);
    postSessionReturnToCompactRef.current = returnToFloating ? false : returnToCompact === true;
    postSessionReturnToFloatingRef.current = returnToFloating === true;
    postSessionBreakUntilRef.current = 0;
    postSessionBreakPromptPendingRef.current = false;
    updateReentryBreakReturnAvailable(false);
    setFloatingBreakState({ open: false });
  ...
  }, [buildPostSessionResumeCandidate, clearCompactSessionCues, closeFloatingReentryPrompt, mode, persistIdleTimerSnapshot, persistableTaskPlan, setFloatingBreakState, updateReentryBreakReturnAvailable]);
  openPostSessionTransitionRef.current = openPostSessionTransition;
  ...
  const closePostSessionSurface = useCallback(({ keepFloatingBreakOpen = false } = {}) => {
    ...
    updateReentryBreakReturnAvailable(false);
    closeFloatingReentryPrompt();
    if (!keepFloatingBreakOpen) {
      setFloatingBreakState({ open: false });
    }
  }, [closeFloatingReentryPrompt, setFloatingBreakState, updateReentryBreakReturnAvailable]);

  const ensurePostSessionSessionId = useCallback(async () => {
    if (postSessionResumeCandidate?.sessionId) {
      return postSessionResumeCandidate.sessionId;
    }
    ...
    try {
      ...
      if (nextSessionId) {
        setPostSessionResumeCandidate((prev) => (
          prev ? { ...prev, sessionId: nextSessionId } : prev
        ));
        await loadSessions();
      }
      return nextSessionId;
    } catch (error) {
      console.error('Failed to create a post-session record:', error);
      return null;
    }
  }, [loadSessions, postSessionResumeCandidate]);

  const updatePostSessionSession = useCallback(async ({
    completed,
    kept,
    recap,
    nextSteps,
  }) => {
    const sessionId = await ensurePostSessionSessionId();
    if (!sessionId) return null;

    const splitNotes = normalizeSplitNotes({ recap, nextSteps }, {
      recap: postSessionResumeCandidate?.recap,
      nextSteps: postSessionResumeCandidate?.nextSteps,
    });
    ...
    try {
      ...
      console.error('Failed to update a post-session record:', error);
      return sessionId;
    }
  ...
  const savePausedPostSessionChoice = useCallback(async ({
    ...
    const taskText = clampTaskText(postSessionResumeCandidate.taskText).trim();
    if (!taskText) return null;
    ...
    try {
      if (sessionId) {
        const updated = await SessionStore.update(sessionId, sessionPatch);
        savedSessionId = updated?.id || null;
      }
      ...
      console.error('Failed to save paused post-session choice:', error);
  ...
  const handleCompleteFromResumeCandidate = useCallback(async (notes = {}) => {
    const candidate = reentryResumeCandidateRef.current;
    const completedSessionId = await completeResumeCandidate(notes, candidate);
    await finishDirectCompletion({
      completedSessionId,
      durationMin: (candidate?.completedMinutes || 0) || ((candidate?.carryoverSeconds || 0) / 60),
      source: 'resume_prompt',
      completedMode: candidate?.completedMode || candidate?.mode || mode,
      bringToFront: true,
    });
  ...
  const handleTakePostSessionBreak = useCallback(async (payload = {}) => {
    ...
    const requestedBreakMinutes = typeof payload === 'object' && payload !== null
      ? payload.minutes
      : payload;
    const showTimer = typeof payload === 'object' && payload !== null
      ? payload.showTimer !== false
      : true;
    const breakMinutes = clampSessionMinutes(requestedBreakMinutes, 5);
    const breakEndsAt = Date.now() + (breakMinutes * 60 * 1000);
    const taskAlreadyCompleted = isCompletedPostSessionCandidate(postSessionResumeCandidate);
    postSessionLastBreakOptionsRef.current = { minutes: breakMinutes, showTimer };
    updateReentryBreakReturnAvailable(false);
  ...
  useEffect(() => {
    ...
    if (postSessionIsPauseWrap) return;
    triggerConfetti(1200);
  }, [postSessionIsPauseWrap, showPostSessionPrompt, triggerConfetti]);

  useEffect(() => {
    let cancelled = false;

    if (!showPostSessionPrompt || !postSessionResumeCandidate?.sessionId) {
      setPostSessionFeedbackEnabled(false);
      return undefined;
    }
    ...
    void (async () => {
      try {
        const settings = await window.electronAPI.storeGet('settings') || {};
        const suppressedUntil = settings.postSessionFeedbackSuppressedUntil
          ? new Date(settings.postSessionFeedbackSuppressedUntil).getTime()
          : 0;
        const sessionCount = Array.isArray(sessions) ? sessions.length : 0;
        const matchingSession = sessions.find((session) => session.id === postSessionResumeCandidate.sessionId) || null;
        const nextEnabled = sessionCount >= 2
          && (!suppressedUntil || suppressedUntil <= Date.now())
          && !matchingSession?.sessionFeedback;
        if (!cancelled) {
          setPostSessionFeedbackEnabled(nextEnabled);
        }
      } catch (error) {
        console.error('Failed to load post-session feedback eligibility:', error);
        if (!cancelled) {
          setPostSessionFeedbackEnabled(false);
        }
      }
  ...
  }, [postSessionResumeCandidate?.sessionId, sessions, showPostSessionPrompt]);

  const handlePostSessionFeedbackDismiss = useCallback(async () => {
    try {
      const settings = await window.electronAPI.storeGet('settings') || {};
      const currentStreak = Math.max(0, Number(settings.postSessionFeedbackSkippedStreak) || 0);
      const nextStreak = currentStreak + 1;
      let suppressedUntil = null;
      if (nextStreak >= 4) {
        suppressedUntil = new Date(Date.now() + (30 * 24 * 60 * 60 * 1000)).toISOString();
      } else if (nextStreak >= 3) {
        suppressedUntil = new Date(Date.now() + (14 * 24 * 60 * 60 * 1000)).toISOString();
      }
      await Promise.all([
        window.electronAPI.storeSet('settings.postSessionFeedbackSkippedStreak', nextStreak),
        window.electronAPI.storeSet('settings.postSessionFeedbackSuppressedUntil', suppressedUntil),
      ]);
    } catch (error) {
      console.error('Failed to persist post-session feedback dismissal:', error);
    }
  }, []);

  const handlePostSessionFeedbackSelect = useCallback(async (feedback) => {
    const sessionId = await ensurePostSessionSessionId();
    await captureSessionFeedback(feedback, {
      surface: 'post_session_wrap',
      completionType: 'wrap',
      sessionId,
      sessionDurationMinutes: postSessionResumeCandidate?.completedMinutes || 0,
      sessionMode: postSessionResumeCandidate?.completedMode || mode,
    });
    try {
      await Promise.all([
        window.electronAPI.storeSet('settings.postSessionFeedbackSkippedStreak', 0),
        window.electronAPI.storeSet('settings.postSessionFeedbackSuppressedUntil', null),
      ]);
    } catch (error) {
      console.error('Failed to reset post-session feedback suppression state:', error);
    }
  }, [captureSessionFeedback, ensurePostSessionSessionId, mode, postSessionResumeCandidate]);
  ...
  const handleSaveSessionNotes = async (notes) => {
    ...
    if (postAction === 'resume-later') {
      ...
      openPostSessionTransition({
        ...
        sessionId: endedSessionId,
        carryoverSeconds: 0,
        completedMinutes: pendingSession?.duration || 0,
        completedMode: mode,
      });
      resetSessionFeedbackFlow();
      return;
    ...
    if (postAction === 'complete-direct') {
      const completionMeta = directCompletionNotesSourceRef.current || {};
      directCompletionNotesSourceRef.current = null;
      const completedSessionId = await saveSessionWithNotes(splitNotes);
      setShowNotesModal(false);
      setSessionNotesMode('complete');
      sessionToSave.current = null;
      await finishDirectCompletion({
        completedSessionId,
        durationMin: completionMeta.durationMin ?? pendingSession?.duration ?? 0,
        source: completionMeta.source || 'direct',
        completedMode: completionMeta.completedMode || mode,
        bringToFront: completionMeta.bringToFront === true,
      });
      return;
    }
  ...
  const handleSkipSessionNotes = async () => {
    ...
    if (postAction === 'complete-direct') {
      const completionMeta = directCompletionNotesSourceRef.current || {};
      directCompletionNotesSourceRef.current = null;
      const completedSessionId = await saveSessionWithNotes({ recap: '', nextSteps: '' });
      setShowNotesModal(false);
      setSessionNotesMode('complete');
      sessionToSave.current = null;
      await finishDirectCompletion({
  ...
  const finalizeTimeUpEndSession = useCallback(async () => {
    ...
    const returnToCompact = timeUpReturnToCompactRef.current;
    const returnToFloating = timeUpReturnToFloatingRef.current;
    setShowTimeUpModal(false);
    timeUpHandledRef.current = true;
    timeUpReturnToCompactRef.current = false;
    timeUpReturnToFloatingRef.current = false;
    ...
    clearCompactSessionCues();
    const durationMin = sessionToSave.current?.duration || (initialTime / 60);
    sessionToSave.current = {
      duration: durationMin,
      completed: false,
      kept: false,
  ...
  }, [clearCompactSessionCues, contextNotes, currentSessionId, initialTime, nextStepsNotes, openPostSessionTransition, saveSessionWithNotes, task]);
  ...
  const prepareTaskForStartChooser = useCallback(({
    ...
    startChooserReturnToCompactRef.current = typeof returnToCompact === 'boolean'
      ? returnToCompact
      : null;
    elapsedBeforeRunRef.current = 0;
  ...
  const handleCloseTaskPreview = useCallback(() => {
    setShowTaskPreview(false);
    setPreviewUseTaskEnabled(true);
    setPreviewRestoreEnabled(false);
    const reopenedPrevModal = popAndOpenPrevModal();
    if (!reopenedPrevModal && showSurfaceReentryPromptRef.current) {
      reentryReturnAfterModalRef.current = false;
      reentryForceRestoreAfterModalRef.current = false;
      setReentryReturnAfterModal(false);
      return;
    ...
    const shouldForceRestoreIdleReentry = reentryForceRestoreAfterModalRef.current
      ...
      && !isTimerVisible
      && reentrySurfaceStage === 'task-entry';
    if (!reopenedPrevModal) {
      restoreReentryPromptAfterModal(shouldForceRestoreIdleReentry);
    }
  }, [isRunning, isTimerVisible, popAndOpenPrevModal, reentrySurfaceStage, restoreReentryPromptAfterModal, task]);
  ...
  useEffect(() => {
    ...
    isTimerVisible,
    persistableTaskPlan,
    taskPlanCompletionDismissedFor,
    hasSavedContext,
    checkInState,
    checkInMessage,
    showSettings,
  ...
  const handleTaskPlanTransitionBreak = () => {
    setTaskPlanTransition(null);
    openPostSessionTransitionRef.current({
      taskText: task,
      recap: contextNotes,
  ...
  const handleTaskPlanTransitionSaveLater = () => {
    setTaskPlanTransition(null);
    persistIdleTimerSnapshot({
      taskText: task,
      recapText: contextNotes,
    ...
    return (
      ...
      <div className={`app-container pill-mode electron-draggable${compactTransitioning ? ' pill-mode--transitioning' : ''}`}>
        <CompactMode
          ...
          onReentryBackToBreakMode={handleBackToBreakMode}
          onReentryInteraction={handleReentryPromptInteraction}
        />
        <CheckInPromptPopup
            ...
            <div className={`focus-stage__surface${showPostSessionPrompt ? ' focus-stage__surface--post-session' : ''}${showSurfaceReentryPrompt ? ' focus-stage__surface--reentry' : ''}`}>
              {showPomodoroBreakPanel ? (
                <PomodoroBreakPanel
                  ...
                  onChooseWorkTime={openPomodoroWorkTimeChooser}
                ...
                <div className="post-session-stage">
                  <PostSessionPrompt
                    ...
                    surfaceKind={postSessionSurfaceKind}
                ...
                <TaskPlanTransitionPrompt
                  completedTask={taskPlanTransition.completedTask}
                  nextTask={taskPlanTransition.nextTask}
                  onContinue={handleTaskPlanContinueNext}
                  onTakeBreak={handleTaskPlanTransitionBreak}
                  onSaveLater={handleTaskPlanTransitionSaveLater}
                />
              ) : (
                <div className="draft-task-builder" data-testid={showDraftTaskPlanBuilder ? 'session-builder' : undefined}>
                  <TaskInput
                    ref={taskInputRef}
                    task={task}
                    ...
                    helperText={visibleFullScreenTaskHelper}
