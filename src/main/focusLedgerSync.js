...
function createFocusLedgerSyncService({
  ...
  function isFocusLedgerEnabled() {
    const settings = store.get('focusInsightsSettings', {})
    return settings && typeof settings === 'object' && settings.enabled === true
  }

  async function enqueueItems(rawItems, reason = 'manual-enqueue') {
    if (!isFocusLedgerEnabled()) {
